...
CONTACT_SIGNAL_RE = re.compile(
    r"(мой\s+(?:телефон|номер|telegram|телеграм|whatsapp|ватсап|контакт)|"
    r"телефон\s+для\s+связи|номер\s+для\s+связи|"
    r"вот\s+(?:мой\s+)?(?:телефон|номер|telegram|телеграм|whatsapp|ватсап|контакт)|"
