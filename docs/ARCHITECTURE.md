...
Горячие лиды описаны отдельно в [HOT_LEADS.md](HOT_LEADS.md). Hot-lead
classification идёт параллельно reply action и не меняет
`REPLY_TEXT / IGNORE / MANUAL`: сначала дешёвый deterministic prefilter,
затем LLM только для кандидатов, после чего результат дедуплицируется в SQLite
и при наличии конфигурации отправляется в Telegram.
